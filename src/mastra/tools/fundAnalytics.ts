...
export const computeInvestmentAnalyticsTool = createTool({
  ...
  description: 'Performs investment analytics on mutual funds. Supports three operations: PERIOD_RETURN calculates a fund\'s performance over a date range, HOLDING_RETURN calculates realized return/P&L on user holdings (the database stores all purchase dates, purchase NAVs, and units, which the tool automatically queries), and FUND_RANKING ranks all funds by period return.',
  inputSchema: z.object({
    operation: z.enum(['PERIOD_RETURN', 'HOLDING_RETURN', 'FUND_RANKING'])
      .describe('PERIOD_RETURN: fund performance over a date range. HOLDING_RETURN: realized return/P&L on user holdings (looks up purchase dates/NAVs automatically from the database; never ask user for purchase dates or dates bought). FUND_RANKING: rank all funds by period return.'),
    ...
    startDate: z.string().optional().describe('Start date for period return calculation (YYYY-MM-DD). Required for PERIOD_RETURN and FUND_RANKING. Ignore/omit for HOLDING_RETURN.'),
    endDate: z.string().optional().describe('End date for period return calculation (YYYY-MM-DD). Required for PERIOD_RETURN and FUND_RANKING. Ignore/omit for HOLDING_RETURN.'),
  }),
